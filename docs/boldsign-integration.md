...
- All source files are stored on the packet: `form_packets.storage_paths` (jsonb `[{ path, name }]`, migration 0022) holds the ordered list; `storage_path` stays the primary/first for back-compat. "Get Forms" downloads every file (one PDF as itself, several as one zip — `src/lib/packetDownload.js`, shared by the Form Library and the pipeline's Required Forms panel). If 0022 hasn't been applied, `FormLibrary`'s save drops `storage_paths` only for a **one-file** packet and refuses a multi-file one — it used to drop it for every packet, silently, which left rows naming only file 1 of an upload. Get Forms rebuilds such a row's upload from the bucket (`uploadBatch()`), and migration 0056 writes the full list back into the row.
