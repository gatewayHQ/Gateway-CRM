-- ═════════════════════════════════════════════════════════════════════════════
-- 0018 — Multi-contact deals/properties + new contact sources
--
-- 1. `contacts.source` CHECK gains 'sign call' and 'friends & family'
--    (requested 2026-07: sign-call leads and friends-&-family referrals were
--    being bucketed under 'other').
-- 2. New junction tables `deal_contacts` and `property_contacts` so a deal or
--    property can carry more than one contact (husband & wife, co-buyers,
--    co-owners). `deals.contact_id` / `properties.linked_contact_id` are
--    unchanged and remain the PRIMARY contact — the junction rows hold the
--    additional ones, so every existing feature that reads the single contact
--    keeps working.
--
...
-- ── 1. contacts.source: add 'sign call' + 'friends & family' ────────────────
alter table contacts drop constraint if exists contacts_source_check;
alter table contacts add  constraint contacts_source_check
  check (source in ('referral','friends & family','website','open house','social',
                    'cold call','sign call','team','paid service','other'));

-- ── 2a. deal_contacts — additional contacts on a deal ───────────────────────
...
-- helper yet), fall back to allow_all — matching the pre-0011 posture of every
...
-- ── 2b. property_contacts — additional contacts on a property ───────────────
...
-- properties themselves are allow_all (public landing page reads — see
-- migrations/README "Known follow-ups"); the link rows match that posture.
