...
| 2026-07-07 | `2026-07-07_esign_transaction_layer.sql` | **APPLIED** — creates `boldsign_documents`, `audit_log`, `document_versions`, `closing_packets`, `agent_nudges`, the `deals.review_*` columns, and the `closing-packets` bucket. Section 8 also drops the legacy `docusign_envelopes`/`docusign_field_templates` tables. |
| 2026-07-08 | `2026-07-08_boldsign_phase1.sql` | **APPLIED** — creates `boldsign_sender_identities` (per-agent send-on-behalf delegation) and `boldsign_templates` (reusable docs + prefill tokens). |
...
| 2026-08-09 | `2026-08-09_boldsign_go_live.sql` | **PENDING — APPLY BEFORE SWITCHING `BOLDSIGN_API_KEY` TO THE LIVE KEY.** Unique index on `boldsign_documents.document_id` (a duplicate made every server-side `maybeSingle()` lookup throw; in the webhook that was swallowed as a 200, so BoldSign never redelivered and the document stopped updating — including the Completed event that archives the signed PDF and audit trail). Also makes `form_packets` writes admin-only (they were open to every authenticated agent, who could delete a state-required form or repoint its BoldSign template); reads stay open. Duplicates are reported, never auto-deleted. Idempotent. |
