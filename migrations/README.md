...
| 0016 | `0016_deal_owner_guard.sql` | (1) Rewrites the `deals` policy to use direct predicates (`app_deal_visible`) instead of a self-referential `id in (select app_visible_deal_ids())` — the latter rejected the "Start Deal" `INSERT ... RETURNING` for every agent while plain inserts worked. (2) Restores `app_is_admin()` to honor the `is_admin` flag (drift where a flagged admin whose role lacked "admin" was treated as a plain agent). (3) BEFORE INSERT owner-guard trigger: defaults a missing owner to the creator and turns a disallowed owner into a plain-English 42501 | **Yes — "Start Deal" works again; flagged admins regain write access; ownerless non-admin inserts land owned by the creator** | Anytime after 0011 |
