...
| 0043 | `0043_om_gated_download.sql` | Gated Offering Memorandum downloads on QR landing pages: the **private** `campaign-oms` storage bucket, `mailing_om_requests` (who unlocked which OM, with the scan that brought them), and `mailing_leads.om_requested` | No (additive; campaigns with no OM attached render exactly as before) | With the OM-gate app deploy |
