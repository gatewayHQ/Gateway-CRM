...
| 0057 | `0057_insert_returning_rls.sql` | **"new row violates row-level security policy for table contacts" when a non-admin agent adds a contact.** The app saves with `.insert().select()` (INSERT ... RETURNING), and Postgres checks a returned row against the policy's USING as well as its WITH CHECK. 0055 made the contacts read rule `id in (select app_visible_contact_ids())` — a lookup on `contacts` that reads the statement's starting snapshot, which cannot contain the row being inserted — so the new row failed its own read rule and the insert rolled back. `deals_agent_scope` has had the same shape since 0011. This puts the row-local arms WITH CHECK already uses (`app_is_admin()`, owner in `app_visible_agent_ids(...)`) in front of the lookup in USING | **Yes, only for rows being inserted:** an agent can now read back the contact or deal they were just allowed to create. Every added arm is already an arm of `app_visible_contact_ids()` / `app_visible_deal_ids()`, so visibility of existing rows is unchanged. WITH CHECK is unchanged | **Now.** Idempotent; apply after 0055 |
