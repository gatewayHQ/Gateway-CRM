...
| 0016 | `0016_deal_owner_guard.sql` | (1) Restores `app_is_admin()` to honor the `is_admin` flag (repairs drift where a flagged admin whose role lacked "admin" was treated as a plain agent — the actual cause of the "Start Deal" RLS failure in production). (2) BEFORE INSERT trigger on `deals`: defaults a missing owner to the creator and turns a disallowed owner into a plain-English 42501 error | **Yes — flagged admins regain firm-wide write access; ownerless non-admin deal inserts now succeed (owned by the creator)** | Anytime after 0011 |
