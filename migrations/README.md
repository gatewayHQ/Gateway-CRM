...
| 0030 | `0030_deals_reconcile_prod.sql` | **Fixes commercial-deal creation for all agents.** One idempotent block reconciling live `deals` to the repo: ensures all form columns exist, widens the stage CHECK to all 16 tokens (commercial + residential), resets the RLS policy set so INSERT is gated on identity (not the impossible "already visible" check), and guarantees the owner-guard / primary-link / closed_at triggers (retiring the redundant + duplicate ones) | **Yes — restores service.** Paste this one block in Supabase | Run now |
