...
| 0023 | `0023_mailing_list_and_profile_rbac.sql` | Mailing-list landing pages (`mailing_subscribers`, `landing_type = 'mailing'`) + profile RBAC: `agents` loses `allow_all`, writes are scoped to self/admin and a trigger freezes privileged columns (`is_admin`, `role`, split/cap) for non-admins | **Yes — a non-admin can no longer edit colleagues' profiles** | With the Mailing List / profile RBAC deploy |
| 0024 | `0024_deal_agent_compensation.sql` | Agent-settable compensation on the deal: `deals.agent_comp_type` (`rate`/`flat`), `agent_comp_rate_pct`, `agent_comp_flat`, their CHECK guards, and a trigger making the fields write-once for agents (only admins/the service key may change a value that is already set) | No (additive; every existing deal keeps the firm default rate until an agent sets a value) | With the agent-compensation deploy |
| 0025 | `0025_deal_co_agents.sql` | Dual-agent deals: `deals.co_agent_ids uuid[]` (the deal-level agent roster, so `roster(deal) = [agent_id] + co_agent_ids`), the `deals_co_agents_exclude_primary` CHECK, a GIN index for the co-listed lookup, and a backfill from each deal's linked property. Also strips properties that list their own `assigned_agent_id` as a co-agent | No (additive; reads only become more complete, and the app falls back to the linked property until it is run) | With the dual-agent deploy |
