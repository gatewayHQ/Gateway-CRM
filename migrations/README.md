...
| 0055 | `0055_fix_deal_access_request.sql` | **Fixes `app_request_deal_access()`, which could never run.** The version in 0054 names five columns and supplies four — `deal_id` was dropped — so every call raised `42601: INSERT has more target columns than expressions` and the "Ask <agent> to add me" button failed with a toast instead of notifying anyone. Caught by `scripts/db-verify/deal_sharing_matrix.sql`, which calls the function rather than reading it | Yes — the access-request button starts working. Nothing else in 0054 was affected (duplicate detection, the whitespace trim and the unique index all ran fine) | **Now, if 0054 is applied.** One statement |
