...
| 0024 | `0024_tagging_and_sharing.sql` | Replaces the three disagreeing "co-agent" sources with one source of truth: `deal_agents` (primary/additional tags) drives ALL deal visibility, decoupled from commission payroll. Adds `sharing_groups`/`_members`/`_records` for cross-team partner sharing of specific contacts/properties, and `visibility_settings` (brokerage/team/user) for opt-in team overrides. Backfills tags from owner + `property.details.co_agent_ids` + `commissions.participants` + legacy `co_agent_ids`; grandfathers existing team sharing; flips `team_splits.share_*` defaults to `false`. Refreshes `app_visible_deal_ids()` to read tags | Yes — refreshing the visibility function changes what non-admins see; backfill runs first so it's continuous. Review the chip-vs-RLS diff query in the file before running | With the tagging-UI app build |

> Note the numeric order vs. recommended run order: **0001 → 0003 → 0004 → 0005 → 0006 → 0007 → 0002 (Phase A) → 0008 → 0009 → 0010 → 0011 (Phase A, then Phase B after verification) → … → 0024 (after 0011)**.
> 0024 supersedes 0011's deal-visibility model: it re-points `app_visible_deal_ids()` at the new `deal_agents` tag table. Run it after 0011 and deploy the matching tagging-UI app build alongside it. See `docs/permissions-redesign.md` for the full design.
