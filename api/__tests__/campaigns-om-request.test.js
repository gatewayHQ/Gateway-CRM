 ...
 *      migration 0043; on a database that hasn't applied it, the visitor still
...
let rejectFirstContact  // simulate contacts.source's pre-0043 CHECK rejecting the insert
...
describe('?action=om_request — trading contact details for the OM', () => {
  ...
  it('still delivers the download and the lead on a pre-0043 database', async () => {
  ...
  it('still creates the contact when the source value is not yet a legal one', async () => {
    // Pre-0043, contacts.source's CHECK rejected 'om-download' and the insert
