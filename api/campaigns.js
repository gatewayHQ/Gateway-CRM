...
// The OM lives in the PRIVATE `campaign-oms` bucket (migration 0045). Nothing
...
async function captureLeadAndContact({
  ...
  // insert: on a database where 0031 / 0045 haven't been applied those columns
  ...
  try {
    if (cleanEmail || phone) {
      ...
      if (!contactId) {
        ...
        // Both of those sources only became legal values in migration 0045;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Returns [] rather than an error on a pre-0045 database so the Campaigns
