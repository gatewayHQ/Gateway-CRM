...
function TaskDrawer({ open, onClose, task, agents, contacts, deals, onSave }) {
  const blank = { title:'', type:'follow-up', priority:'medium', due_date:'', contact_id:'', deal_id:'', agent_id:'', notes:'', completed:false, recurrence:'none' }
  const [form, setForm] = useState(blank)
  const [errors, setErrors] = useState({})
  const [saving, setSaving] = useState(false)

  React.useEffect(() => {
    setForm(task ? { ...task, due_date: task.due_date ? task.due_date.slice(0,16) : '', recurrence: task.recurrence || 'none' } : blank)
    setErrors({})
  }, [task, open])
    ...
    <Drawer open={open} onClose={onClose} title={task?.id ? 'Edit Task' : 'Add Task'}>
      <div className="drawer__body">
        ...
        <div className="form-group">
          ...
          <select className="form-control" value={form.recurrence||'none'} onChange={e=>set('recurrence',e.target.value)}>
    ...
    <div className={`task-row${task.completed?' completed':''}`}>
      ...
      <div className="task-meta">
        ...
        {task.recurrence && task.recurrence !== 'none' && (
          <span style={{ fontSize:10, fontWeight:700, color:'var(--gw-azure)', background:'var(--gw-sky)', padding:'1px 5px', borderRadius:4 }} title={`Repeats ${task.recurrence}`}>↻</span>
        )}
...
export default function TasksPage({ db, setDb, activeAgent }) {
  ...
  const nextRecurrenceDate = (dueDate, recurrence) => {
  ...
  const toggle = useCallback(async (task) => {
    ...
    // Auto-create next instance for recurring tasks when completing
    if (completed && task.recurrence && task.recurrence !== 'none' && task.due_date) {
      ...
      if (nextDue) {
        ...
        if (newTask) {
          ...
          pushToast(`Task completed ✓ · Next: ${new Date(nextDue).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`)
          return
        }
      }
    }
