...
import { templateEditorUrl, selectionsSpec } from '../lib/services/boldsign.js'
import BoldSignFrame from '../components/BoldSignFrame.jsx'
import { deliverPacket, packetFiles, downloadBlob } from '../lib/packetDownload.js'
...
function UploadModal({ packet, onClose, onSaved }) {
  ...
  // Every tick box on this packet's template, in the order it PRINTS, as a
  // markdown table to name them from. BoldSign captions an unnamed box with its
  // own auto id (`CheckBox1`, `CheckBox11`) and hands fields back in placement
  // order, so the send screen's Selections list used to be unnameable without
  // clicking each row in the editor — and naming one wrong locks the opposite
  // term into a signed agreement.
  //
  // Generated by the SERVER, which already holds the BoldSign key, so producing
  // this report never requires the account's signing key on anybody's machine.
  const downloadSelectionsSpec = async () => {
    const templateId = (form.boldsign_template_id || '').trim()
    if (!templateId) { pushToast('Link a BoldSign template first', 'error'); return }
    setSpecBusy(true)
    try {
      const { markdown, count } = await selectionsSpec(templateId)
      if (!markdown) { pushToast('BoldSign returned no fields for this template', 'error'); return }
      const slug = (form.name || 'template').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 60) || 'template'
      downloadBlob(new Blob([markdown], { type: 'text/markdown' }), `${slug}-selections.md`)
      // Zero is worth saying out loud: it means the packet has no tick boxes at
      // all, which is a different situation from a download that went wrong.
      pushToast(count === 0 ? 'Saved — this template has no tick boxes' : 'Selections spec saved', 'success')
    } catch (e) {
      pushToast(`Could not build the selections spec: ${e.message}`, 'error')
    } finally {
      setSpecBusy(false)
    }
  }
          ...
          <div className="form-group">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
              <button
                type="button"
                className="btn btn--secondary btn--sm"
                onClick={downloadSelectionsSpec}
                disabled={specBusy || !form.boldsign_template_id}
                title="Download this template's tick boxes in the order they print, as a table to name them from"
              >
                {specBusy ? 'Reading template…' : 'Download selections spec'}
              </button>
              <span style={{ fontSize: 11, color: 'var(--gw-mist)' }}>
                Every tick box in printed order, with its id, page and current state — so the send
                screen&rsquo;s Selections list can be named against the packet.
              </span>
            </div>
          </div>
          <div className="form-row">
