...
import { formatCurrency, formatCurrencyExact, formatDate, STAGE_LABELS } from '../lib/helpers.js'
...
export default function MyEarnings({ activeAgent }) {
  ...
  const dealRow = (d) => (
    <tr key={d.deal_id} style={{ borderTop: '1px solid var(--gw-border)' }}>
      ...
      <td style={{ padding: '9px 12px', whiteSpace: 'nowrap', fontWeight: 700, color: 'var(--gw-green)' }}>{formatCurrencyExact(d.take)}</td>
      <td style={{ padding: '9px 12px', whiteSpace: 'nowrap', color: 'var(--gw-mist)' }}>{d.split_pct != null ? `${d.split_pct}%` : '—'}</td>
      <td style={{ padding: '9px 12px', whiteSpace: 'nowrap', color: 'var(--gw-mist)' }}>{d.fees > 0 ? formatCurrencyExact(d.fees) : '—'}</td>
    ...
    <div className="page-content">
      ...
      <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(4,1fr)', marginBottom: 16 }}>
        <div className="stat-card" style={{ borderLeft: '3px solid var(--gw-green)' }}>
          <div className="stat-card__value" style={{ color: 'var(--gw-green)' }}>{formatCurrencyExact(ytd.take)}</div>
        ...
        <div className="stat-card">
          <div className="stat-card__value">{formatCurrencyExact(pipelineTake)}</div>
          <div className="stat-card__label">Projected from open deals</div>
        </div>
        <div className="stat-card">
          <div className="stat-card__value">{formatCurrencyExact(cap.ytd_fees)}</div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--gw-mist)' }}>
            {cap.prepaid
              ...
              : cap.amount > 0
                ? `${formatCurrencyExact(cap.ytd_cap_paid)} of ${formatCurrency(cap.amount)} · resets ${cap.anniversary ? formatDate(cap.anniversary).replace(/, \d{4}$/, '') : 'Jan 1'}`
