...
-- ── DEAL VISIBILITY MODEL (deal_agents) — see migration 0029 ─────────────────
-- Ownership lives in deal_agents (a row per agent-on-deal, role primary|
-- additional); team-level visibility is governed by visibility_settings via
-- app_team_deal_visibility(). This replaced the earlier deals.agent_id +
-- commissions.participants model.
...
-- Per-team / brokerage visibility resolver (category override → team → broker → off).
...
-- Policies for the two model tables (deal_agents_scope needs app_visible_deal_ids above).
...
-- DEALS — visibility via the deal_agents model (see 0029). Two triggers keep it
-- coherent: app_deal_owner_guard stamps/validates the owner on insert, and
-- sync_primary_deal_agent mirrors deals.agent_id into deal_agents(role=primary).
...
-- Owner guard: stamp a null owner to the creator, and enforce "self or a sharing
-- teammate" with human-readable errors. Admins / service key bypass.
